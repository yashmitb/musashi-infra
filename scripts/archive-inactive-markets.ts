...
try {
  const candidateRows = await sql.unsafe<{ archive_candidates: string }[]>(`select count(*)::bigint as archive_candidates
    from markets m
  ...
  if (execute) {
    while (true) {
      const moved = await sql.unsafe<{ archived_count: string; deleted_count: string }[]>(`with candidate_batch as (
        select *
          from markets m
         where m.platform = 'kalshi'
           and m.status = 'closed'

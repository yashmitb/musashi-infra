...
const MusashiEnvSchema = z.object({
  ...
  settlesAtBackfillMaxRuns: z.coerce.number().int().positive().default(3),
  settlesAtBackfillMaxDurationMs: z.coerce.number().int().positive().default(900000),
  settlesAtBackfillMaxMarkets: z.coerce.number().int().positive().default(400),
  settlesAtBackfillFetchConcurrency: z.coerce.number().int().positive().default(3),
  settlesAtBackfillWorkerRateLimitMs: z.coerce.number().int().nonnegative().default(200),
...
export function getEnv(): MusashiEnv {
  const rawEnv = {
    ...
    settlesAtBackfillMaxRuns: process.env.SETTLES_AT_BACKFILL_MAX_RUNS,
    settlesAtBackfillMaxDurationMs: process.env.SETTLES_AT_BACKFILL_MAX_DURATION_MS,
    settlesAtBackfillMaxMarkets: process.env.SETTLES_AT_BACKFILL_MAX_MARKETS,
    settlesAtBackfillFetchConcurrency: process.env.SETTLES_AT_BACKFILL_FETCH_CONCURRENCY,
    settlesAtBackfillWorkerRateLimitMs: process.env.SETTLES_AT_BACKFILL_WORKER_RATE_LIMIT_MS,
